...
| The product wears its design | Full design system applied to the dApp itself (not a prototype): landing, directory, agent pages, permission sheet, dashboard, receipts, fire flow |
| One-command experiences | `npm run app` (walletless Grant), `npm run demo` (protocol e2e), `npm run faucet -- --self-test` (wallet funding path) |
| Documentation site | This site: architecture, ZK design, UX, business, roadmap |

## Wave 1 submission checklist 🔜

Demo video recorded · GitHub repository public (with `midnightntwrk` topic) · pitch deck exported · AKINDO
submission filed before **September 16, 2026**.
