...
2. **"Know Your Agent" is becoming a compliance topic.** Bank supervisors have begun asking how KYC extends to
...
2. **No scoped delegation primitive.** Session keys (e.g. EIP-7702) bound *which key* can act, not *what a specific
...
| Stripe + OpenAI ACP | Agent checkout protocol | Merchant/processor sees the buyer; no user-set caps | User-set caps enforced at proof time; receipts verifiable without PII |
...
| zkPassport / Self / Anon Aadhaar | ZK identity credentials (EVM-adjacent) | Prove who you are; no delegation or agent semantics | Composable later for the data-access grant (e.g. passenger name) |
...
proof-enforced delegation** — is where Grant operates. Our strategy is deliberately non-adversarial toward the
...
Rather than claim a slice of a trillion-dollar TAM, our serviceable model is explicitly assumption-based (all
