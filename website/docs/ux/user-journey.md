...
2. **Hire:** tap Hire; the permission sheet slides up; lower the cap if you like; tap Allow. Your wallet signs; a
   proving state explains what is happening ("issuing private credential… never your keys").
...
## How a hired agent actually books

Told with TravelBooker, because it is the first question every skeptic asks:
...
3. **The moment of commitment.** To book, the agent must produce a zero-knowledge proof: *a live credential delegates me;
   book($32) is in scope, under budget, before expiry.* For anything you did not allow, no proof can exist: the
...
passenger name? That is a scoped one-time **data-access grant** on the same permission sheet: selective disclosure
is Midnight's native move.

## Deliberately designed journey states
...
| Agent tries something forbidden | The rejection is celebrated, not hidden: it is the demo of the guarantee |
| After firing | Locked-out state stays visible with its receipts: revocation has a face |
| Coming back tomorrow | Session persists locally (registry address, credentials, receipts), so refresh loses nothing |
