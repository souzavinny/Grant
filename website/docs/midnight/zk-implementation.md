...
Three proof circuits carry the whole protocol. This page explains what each proves, what it discloses, and the two
...
chain learns only an agent-chosen bound (rounded to the hour), never the mandate's actual expiry. We found this
because the Compact compiler's disclosure analysis *rejected* the naive version; the type system taught us the
privacy leak.
...
enforcement the user configured is exactly the enforcement the math applies. Our adversarial test suite includes the
case where the agent inflates its own copy of the cap: the proof fails with `terms do not match commitment`.

## What the compiler enforces for us

Compact's "witness protection" analysis makes any flow of private data to a public sink a compile error unless
explicitly annotated with `disclose()`. Our contract has exactly the disclosures listed above and the compiler will
