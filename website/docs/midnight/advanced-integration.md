...
How Grant plugs into the full Midnight stack, and the four independent verification routes we ran before calling
anything "working."
...
| Proof server | Generates the ZK proofs for issue/authorize/revoke | Local (:6300) — secrets never leave the machine |
...
and the **midnight-verify** agents for mechanical verification. The identity patterns in our contract
...
## The verification stack (four independent routes)
...
checker checks the proof-system artifacts, and the devnet run checks the whole organism. A claim that survives all
four is the closest a hackathon gets to "known correct."
:::
