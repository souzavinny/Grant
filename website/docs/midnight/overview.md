...
Midnight's dual-ledger model — private state on the user's device, ZK proofs bridging to public on-chain state — is
...
## What Midnight specifically gives us

- **Compact:** a TypeScript-adjacent contract language where the compiler's disclosure analysis forces every public
  reveal to be explicit — our privacy accounting is machine-checked, not aspirational.
...
- **Selective disclosure as a native pattern:** the Wave-3 "data-access grant" (disclose one field, e.g. a passenger
...
doesn't support them yet, so AgentPass is a deliberately single-contract design; the constraint shaped the
