...
Our onboarding principle: **nobody should need to learn a word of blockchain to set a spending cap.** The chain
...
| Door | Who it's for | What it requires |
|---|---|---|
...
The wallet gate appears only at the moment it's needed: the first tap on *Hire*. If no wallet is connected, Grant
...
On a local devnet, self-custody users must fund their wallet (our faucet sends NIGHT in one command) and let the
wallet generate DUST for fees — a step the wallet performs because fee-resource registration must be signed by the
key owner. We verified this exact path end-to-end (`npm run faucet -- --self-test`) and documented it in the README.
